...
- **The fields ARE held — all ten, recovered 2026-08-02.** This entry previously read "the fields are not held" and stated that no state survived, that `field_store.py status` reported **0/10 held, 10 MISSING**, and that refilling them needed a 24 GB GPU. That was wrong, and the states were never actually gone — only unfindable. All ten were sitting gitignored in `null-worldtube-private` (`simulations/engine_dogfood/out_*_n192*` and `output/periodic_table/*/out_ehn_relax`), 757 MB each. **Every sha256 matches what this catalog declared in July**, so none is `rederived` — these are the originals, not re-runs. They are now banked in the content-addressed store (`src/soliton_playground/ehn_lab/field_store.py`), CRC-verified on the way in; `status` reports **10/10 held** and exits 0, and `index.json` is tracked so the repo states what it holds. `leg_B5` conforms 10 of 10 against their registrations, reproducing July's linking numbers and knot determinants exactly (`cinquefoil_t25`: lk −5.0 vs −5.0, det [[822, 5]] vs [[822, 5]]). Registration banks a field at the moment it is created, so the losing-track cannot silently recur.
